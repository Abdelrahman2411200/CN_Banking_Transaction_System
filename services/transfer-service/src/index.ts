...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.on('finish', () => {
    ...
    console.info(`[transfer-service] ${requestId} ${req.method} ${req.originalUrl} ${res.statusCode} ${durationMs}ms`);
  });
...
if (require.main === module) {
  void startOutboxPublisher(pool)
    ...
    .catch((error: unknown) => {
      console.error('Failed to start transfer-service outbox publisher', error);
      process.exit(1);
    });
}
