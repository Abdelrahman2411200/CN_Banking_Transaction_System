...
export const fraudRulesEvaluatedTotal = new Counter({
  name: 'fraud_rules_evaluated_total',
  help: 'Fraud rules evaluated by rule name',
  labelNames: ['rule_name'] as const,
  registers: [registry],
});

export const fraudAlertsTriggeredTotal = new Counter({
  name: 'fraud_alerts_triggered_total',
  help: 'Fraud alerts triggered by rule and severity',
  labelNames: ['rule_name', 'severity'] as const,
  registers: [registry],
});

export const fraudEvaluationDurationMs = new Histogram({
  name: 'fraud_evaluation_duration_ms',
  help: 'Fraud evaluation duration in milliseconds',
  registers: [registry],
});
